    ...
    def __init__(self, datasets: dict, transform=None):
        ...
        for k, _ in self.datasets.items():
            for img in self.datasets[k]['images']:
                self.data.append(
                    {
                        'image': self.datasets[k]['dir'].joinpath('input', img),
                        'label': self.datasets[k]['dir'].joinpath('target', img)
                    }
                )
